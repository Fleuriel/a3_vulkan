...
public:
	...
	struct Compute {
		...
		VkSemaphore semaphore{ VK_NULL_HANDLE };						// Execution dependency between compute & graphic submission
		std::vector<VkDescriptorSetLayout> descriptorSetLayout{ }; // 3 layouts
		std::vector<VkDescriptorSet> descriptorSet{ };             // 3 sets
		std::vector<VkPipelineLayout> pipelineLayout{ };           // 3 layouts
		std::vector<VkPipeline> pipelines{ };                      // histogram, cdfscan, applyhisto
		uint32_t queueFamilyIndex;
	} compute;
	...
	vks::Buffer cs_storagebuffer;

	struct alignas(16) {
		alignas(16) uint32_t histoBin[256];
		alignas(16) float cdf[256];
		alignas(16) uint32_t totCnt[2]; // Optional as per your shader design
	} bHistBin;


	void updateStorageBuffers() {
		memset(&bHistBin, 0, sizeof(bHistBin));
		bHistBin.totCnt[0] = textureColorMap.width;
		bHistBin.totCnt[1] = textureColorMap.height;
		memcpy(cs_storagebuffer.mapped, &bHistBin, sizeof(bHistBin));
	}
	...
	void prepareStorageBuffers() {
		VK_CHECK_RESULT(vulkanDevice->createBuffer(
			...
			&cs_storagebuffer, sizeof(bHistBin)));
		VK_CHECK_RESULT(cs_storagebuffer.map());
		updateStorageBuffers();
	}


	VulkanExample() : VkAppBase()
	{
	...
	~VulkanExample()
	{
		if (device) {
			...
			for (auto& layout : compute.pipelineLayout) {
				vkDestroyPipelineLayout(device, layout, nullptr);
			}
			for (auto& descLayout : compute.descriptorSetLayout) {
...
//			imageMemoryBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
			...
			imageMemoryBarrier.dstQueueFamilyIndex = graphics.queueFamilyIndex;
	...
	void buildComputeCommandBuffer()
	{
		...
		// Stage 1: histogram.comp
		vkCmdBindPipeline(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelines[0]);
		vkCmdBindDescriptorSets(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelineLayout[0], 0, 1, &compute.descriptorSet[0], 0, nullptr);
		vkCmdDispatch(compute.commandBuffer, (storageImage.width + 15) / 16, (storageImage.height + 15) / 16, 1);

		// Barrier to sync histogram -> cdfscan
		VkMemoryBarrier barrier = vks::initializers::memoryBarrier();
		barrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT;
		barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
		vkCmdPipelineBarrier(compute.commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, nullptr, 0, nullptr);

		// Stage 2: cdfscan.comp
		vkCmdBindPipeline(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelines[1]);
		vkCmdBindDescriptorSets(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelineLayout[1], 0, 1, &compute.descriptorSet[1], 0, nullptr);
		vkCmdDispatch(compute.commandBuffer, 1, 1, 1);

		// Barrier to sync cdfscan -> applyhisto
		vkCmdPipelineBarrier(compute.commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 1, &barrier, 0, nullptr, 0, nullptr);

		// Stage 3: applyhisto.comp
		vkCmdBindPipeline(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelines[2]);
		vkCmdBindDescriptorSets(compute.commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute.pipelineLayout[2], 0, 1, &compute.descriptorSet[2], 0, nullptr);
		vkCmdDispatch(compute.commandBuffer, (storageImage.width + 15) / 16, (storageImage.height + 15) / 16, 1);
	...
	void setupDescriptorPool()
	{
		std::vector<VkDescriptorPoolSize> poolSizes = {
			...
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 3),  // <-- changed
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 3), // <-- new
		};
	...
	void prepareCompute()
	{
		...
		compute.pipelines.resize(3);

		std::vector<std::string> shaderNames = { "histogram", "cdfscan", "applyhisto" };

		for (int i = 0; i < 3; ++i) {
			std::vector<VkDescriptorSetLayoutBinding> bindings;

			if (shaderNames[i] == "histogram") {
				bindings = {
					vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT, 0),
					vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT, 1)
				};
			}
			else if (shaderNames[i] == "cdfscan") {
				bindings = {
					vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT, 0)
				};
			}
			else if (shaderNames[i] == "applyhisto") {
				bindings = {
					vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT, 0),
			...
			VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = vks::initializers::pipelineLayoutCreateInfo(&compute.descriptorSetLayout[i], 1);
			VK_CHECK_RESULT(vkCreatePipelineLayout(device, &pipelineLayoutCreateInfo, nullptr, &compute.pipelineLayout[i]));

			VkDescriptorSetAllocateInfo allocInfo = vks::initializers::descriptorSetAllocateInfo(descriptorPool, &compute.descriptorSetLayout[i], 1);
			...
			if (shaderNames[i] == "histogram") {
				computeWriteDescriptorSets = {
					vks::initializers::writeDescriptorSet(compute.descriptorSet[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &textureColorMap.descriptor),
					vks::initializers::writeDescriptorSet(compute.descriptorSet[i], VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1, &cs_storagebuffer.descriptor)
				};
			}
			else if (shaderNames[i] == "cdfscan") {
				computeWriteDescriptorSets = {
					vks::initializers::writeDescriptorSet(compute.descriptorSet[i], VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2, &cs_storagebuffer.descriptor)
				};
			}
			else if (shaderNames[i] == "applyhisto") {
				computeWriteDescriptorSets = {
					vks::initializers::writeDescriptorSet(compute.descriptorSet[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0, &textureColorMap.descriptor),
					vks::initializers::writeDescriptorSet(compute.descriptorSet[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1, &storageImage.descriptor),
					vks::initializers::writeDescriptorSet(compute.descriptorSet[i], VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2, &cs_storagebuffer.descriptor)
				};
			}
			...
			// Pipeline creation
			VkComputePipelineCreateInfo computePipelineCreateInfo = vks::initializers::computePipelineCreateInfo(compute.pipelineLayout[i], 0);
			std::string fileName = getShadersPath() + "computeshader/" + shaderNames[i] + ".comp.spv";
			computePipelineCreateInfo.stage = loadShader(fileName, VK_SHADER_STAGE_COMPUTE_BIT);

			VkPipeline pipeline;

			VK_CHECK_RESULT(vkCreateComputePipelines(device, pipelineCache, 1, &computePipelineCreateInfo, nullptr, &compute.pipelines[i]));
	...
	void draw()
	{
		...
		VkPipelineStageFlags graphicsWaitStageMasks[] = { VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT };
	...
	{
		if (overlay->header("Compute Pipeline")) {
			overlay->text("Pipeline: Histogram Equalization");
		}
	}
};
